...
define('AWS_ACCESS_KEY', '/root/.ec2/pk-OTTZ2OZ3UKDVMK7335FOBQ6W2VAVBOI5.pem');
define('AWS_SECRET_ACCESS_CERT', '/root/.ec2/cert-OTTZ2OZ3UKDVMK7335FOBQ6W2VAVBOI5.pem');
    ...
    if (count($snap_arr) > $keep_qty) {
      ...
      foreach ($snap_arr as $s) {
        if ($count <= $keep_qty) {
          ...
          exec($cmd, $out);
          print "** INFO: Output from command : \n";
          foreach ($out as $o) {
            print $o . "\n";
          }
        }
