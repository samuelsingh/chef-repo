...
http_request "add_host" do
  url "http://admin:noj4Ahe@#{zen_srv}:8080/zport/dmd/DeviceLoader/loadDevice?deviceName=#{node[:fqdn]}&devicePath=/Server/Linux&zSnmpCommunity=#{node[:snmpd][:community]}&locationPath=#{node[:domain]}&comments=''"
  action :nothing
end
